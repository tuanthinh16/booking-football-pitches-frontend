...
export const Login = ()=>{
    ...
    const  onLogin = async ()=>{
        ...
        Fdata.append('username',Account.username);
        Fdata.append('password',Account.password);
        try {
            const rs = await loginAPI(Fdata);
            
            if(rs){
                localStorage.setItem('token',rs['data']['token']);
                localStorage.setItem('usr',Account.username);
                history('/');
...
Chào mừng bạn đến với CLB Bóng đá, nơi hội tụ những đam mê và tình yêu dành cho môn thể thao vua. Để truy cập vào trang web và khám phá những nội dung hấp dẫn, vui lòng đăng nhập để trải nghiệm một cách đầy đủ.
                ...
                <h2 id='login-header'>Vui Lòng Điền Thông Tin</h2>
